...
public interface FeatureAttributeValueDAO extends DAO<FeatureAttributeValue>, JpaSpecificationExecutor<FeatureAttributeValue> {
	...
	default List<FeatureAttributeValue> loadByElementAndFeature(int elementId, String featureCode) {
		return findAll(
				belongsToFeature(featureCode),
	...
	default Specification<FeatureAttributeValue> belongsToFeature(String featureCode){
        return (value, cq, cb) -> {
            ...
            return cb.equal(feature.get(Feature.PROP_CODE), featureCode);
        };
	}
